...
| Вход и регистрация | `auth.html`, `register.html` | Cookie-сессия; кнопка «Войти через Яндекс» (OAuth). После входа с живым холдом — на проверку записи, иначе в кабинет |
...
**Вход через Яндекс:** задайте в `.env` (и на сервере) `YANDEX_CLIENT_ID`, `YANDEX_CLIENT_SECRET` и `YANDEX_REDIRECT_URI` из кабинета приложения на https://oauth.yandex.ru/. Redirect URI должен совпадать с зарегистрированным (например `https://nogotochki-test.ru/auth/yandex/callback`). Обмен кода на профиль — в `server/src/domain/yandex-oauth.js`; токен Яндекса не хранится.
