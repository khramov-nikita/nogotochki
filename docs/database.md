...
| 2026-09-09 | В проект добавлен вход через внешний сервис (Яндекс): миграция `008_oauth_clients.sql` (`password_hash` nullable, `provider` / `provider_id`), `POST /api/auth/yandex`, кнопка на экранах входа/регистрации, check `password-login-available` без reset-HTTP. |
...
| 2026-09-10 | **G4:** на шаге 1 записи не было «Дизайн ногтей». Причина — урезанный `seed-dev.js`: в `seedServices` не было `nail_design`, `is_addon` всегда `0`; локальная БД после `db:dev` тоже без добавки. Исправление: в `seed-dev` добавлен `nail_design` (`is_addon=1`) без фиксированного id и привязка к мастеру 1; в текущую SQLite услуга дописана. G4 и связанные (G5–G7, C11, A4–A5) перепроверены. G3 (сертификат) не трогали. |
| 2026-09-10 | **C17:** на переносе не показывалось «Нет свободного времени». Причина — в `cabinet-reschedule.js` дни без окон получали `disabled`, клик игнорировался, empty из `renderSlots` был недостижим; оставался блок «Свободное время» прошлого дня. Исправление: в горизонте пустые дни кликабельны (вид `unavailable` без `disabled`); при смене дня сбрасывается выбранный слот. C17 и C16 перепроверены. |
| 2026-09-10 | **S3:** confirm после чужой записи на слот давал `HOLD_EXPIRED` («не найден»), потому что purge стирал холд до проверки занятости. Исправлено: снимок холда до purge + `SLOT_TAKEN`, если слот занят; в `booking-review.js` fallback на «Это время уже занято», если холд уже стёрт. S3 и связанные S2 / G12 / C11 перепроверены. |
| 2026-09-10 | **G3:** на лендинге не было сертификата / «Условия в студии». Причина — в `seed-dev.js` не было `gift_certificate`; локальная БД после `db:dev` тоже без него. Исправление: в seed-dev добавлен `gift_certificate` (`is_bookable=0`, цены 3000/5000, срок 6 мес.); в текущую SQLite услуга дописана. UI уже различал CTA по флагу. G3 и связанные (G1–G2, G4–G7, C11, A4–A5) перепроверены. |
...
| POST | `/api/auth/yandex` | публичный, rate limit по `req.ip` | Вход/привязка через Яндекс (code → профиль); сессия как у login |
