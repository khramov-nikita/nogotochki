...
**Для следующей сессии ИИ:** прочитай `cloud.md`, этот файл, затем `docs/db-schema.md`. Не выдумывай таблицу свободных слотов. Не открывай SQLite мимо `server/src/db/connection.js`. HTTP API записи — `server/src/http/` и `server/src/domain/`. Черновой тестовый UI — `web/` (Vite, vanilla JS, не продуктовый дизайн). Локальный запуск для человека — `README.md`.
...
| [`web/`](../web/) | Черновой тестовый UI: Vite :5173, proxy `/api` → :3000, hash-router, Bearer в `localStorage` |
| `data/nogotochki.sqlite` | Файл базы (в git не входит) |
| [`README.md`](../README.md) | Запуск с нуля: Node, `.env`, API, тестовая страница |
...
- Кабинет сотрудника как отдельный продукт по-прежнему вне v1. Не добавлять `clients.role`; роли — список в `client_roles`. Тестовый UI мастеру показывает то, что отдаёт `GET /api/appointments`, без отдельного кабинета сотрудника.
- Продуктовый UI (Figma / v0) на этот API ещё не переносили. Черновой тестовый фронт — `web/`.
