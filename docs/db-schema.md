...
|---|---|---|
| 1. Услуги | Записываемые услуги без сертификата; дизайн как добавка; сумма и длительность | `services` где `is_bookable = 1`. Добавка: `is_addon = 1`. |
...
| 5. Готово | Запись создана, статус «ожидает предоплату», CTA в кабинет | `appointments` (`pending_prepayment`) + `appointment_services`. Имя мастера — JOIN `masters`. Сумма и длительность — из `services` по `service_id`. Холд удаляется. Адреса нет. |
...
| Детали / ожидает предоплату | Статус, услуги, сумма, длительность, мастер, дата и время, ссылка на правила | `appointments` + `appointment_services` + JOIN `masters` и `services`. Сумма = Σ `price_rub`, длительность — по `duration_*` услуг. Адреса нет. |
...
Правила шага 1 живут в приложении, не в отдельной таблице: хотя бы одна услуга с `is_bookable = 1` и `is_addon = 0`; добавку нельзя подтвердить одну.
...
Цены и длительности при показе сводки читаются из `services` по `service_id`. Снимков в составе холда и записи нет (§6.18).
...
Сумма и длительность на экранах: JOIN `appointment_services` → `services`, Σ `price_rub` и Σ `duration_*`. Не дублировать итог в строке записи.
...
**Назначение.** Состав визита: какие услуги входят в запись. Название, цена, длительность, флаг добавки читаются из `services` по `service_id`. Снимков нет.

| Поле | Тип | Обязательное | Ключ | Описание |
|---|---|---|---|---|
| `id` | INTEGER | да | PK | |
| `appointment_id` | INTEGER | да | FK → `appointments.id` ON DELETE CASCADE | |
| `service_id` | INTEGER | да | FK → `services.id` ON DELETE RESTRICT | Источник названия, цены и длительности |
| `sort_order` | INTEGER | да | | Порядок выбора в сводке, не копия `services.sort_order` |

«Записаться снова» берёт те же `service_id`. Услугу с прайса нельзя удалить, пока на неё есть записи (`RESTRICT`). Смена цены в каталоге меняет и отображение старых визитов — см. §6.18.
...
| Название, цена, длительность, флаг добавки услуги | `services` (`name`, `price_rub`, `duration_min_minutes`, `duration_max_minutes`, `is_addon`) | `appointment_services.service_id` и `booking_hold_services.service_id` |
...
Если прайс или имя мастера изменятся, карточки старых визитов покажут новые подписи и цены. Занятое время не поедет: оно в `appointments.starts_at` / `ends_at`.
...
### 10.20. Снимки прайса и имени убраны

Можно было копировать цену и имя мастера в запись, чтобы история не плыла.

Это те же поля, что в `services` и `masters`. Источник — справочник, в записи и составе визита только внешние ключи (§6.18). Занятый интервал (`starts_at` / `ends_at`) не снимок прайса: это факт брони. Если каталог изменят, подписи старых карточек обновятся; занятость слота — нет.
