...
|---|---|---|
| Вход | Почта, пароль | Поиск `clients.email`, сверка с `password_hash`. Успех → строка в `sessions`. Аккаунт только через Яндекс (`password_hash` NULL) → ошибка, вход через провайдера. |
| Вход через Яндекс | Кнопка на экранах входа/регистрации | Профиль с почтой и именем от провайдера (или заглушка). Поиск/создание `clients`, привязка `provider` / `provider_id`, сессия как у обычного входа. Токен Яндекса не храним. |
...
| `password_hash` | TEXT | нет | | Только salted scrypt (PHC: `$scrypt$N=…$r=…$p=…$salt$hash`). NULL у аккаунта только через внешний вход (Яндекс). Колонок `password`, `password_plain`, `passwd` **нет** и заводить нельзя. Не bcrypt и не argon2: native-аддоны на BeGet не собираются |
| `display_name` | TEXT | нет | | Имя в профиле. Регистрация его не требует; при входе через Яндекс может прийти из внешнего профиля |
| `provider` | TEXT | нет | | Внешний провайдер входа, сейчас `yandex`. NULL у обычной регистрации |
| `provider_id` | TEXT | нет | UNIQUE вместе с `provider` (частичный индекс, оба NOT NULL) | Id пользователя у провайдера. Токен провайдера **не** храним |
| `created_at` | TEXT | да | | ISO 8601 UTC |
| `updated_at` | TEXT | да | | ISO 8601 UTC |

Повтор пароля на форме регистрации и поля «текущий / новый / повтор» при смене живут только в запросе UI. В таблицу попадает новый `password_hash`. Открытый пароль не пишется ни в `clients`, ни в логи, ни в `password_reset_tokens` (там хеш **токена** ссылки, не пароля). Внешний вход ищет клиента по `email`: существующий аккаунт привязывает `provider` / `provider_id`, новый создаётся с `password_hash = NULL` и ролью `client`.
...
**Пароль.** В `clients` есть `password_hash` (salted scrypt с параметрами в строке) или NULL, если вход только через внешнего провайдера. Поля для открытого пароля нет. Формы входа, регистрации и смены передают пароль в приложение; в SQLite пишется результат `crypto.scrypt`. В `sessions` и `password_reset_tokens` — хеш **токена**, не пароля. Токен Яндекса не хранится.
...
| `clients (provider, provider_id)` UNIQUE WHERE оба NOT NULL | Один внешний id — один аккаунт | Два клиента на один Яндекс-id |
